...
| A character with four rotations | `POST /create-character-with-4-directions` | Background job, collected exactly as v3 is. South, east, north and west from a template-based model rather than a rotation-based one. `image_size` is required here, and references arrive as a `directions` map whose sprites must match it exactly. Carries `shading`, which v3 does not. |
...
| An animation for an existing character | `POST /characters/animations` | Background job per direction. `mode=template` is one generation per direction from a named template, driving the character's skeleton — cheapest, and PixelLab is not currently returning correct frames for it; `mode=v3` animates from an action description at one generation per frame per direction, and is what PixelLab recommends; `mode=pro` costs twenty to forty. In `v3`, `custom_start_frame` replaces the character's rotation as the frame the motion starts on and `end_frame` switches the call into interpolation toward a target pose — one frame per call, so one direction per call. |
| A motion description written from the frame it starts on | `POST /enhance-animation-v3-prompt` | Synchronous, about 0.05 generations. Takes `first_frame` and `action` and returns `enhanced_prompt`; with `last_frame` it describes the interpolated motion between two poses. The cheapest quality lever on this surface. |
...
| The frames between two poses | `POST /interpolation-v2` | Background job, Pro pricing. Start pose, end pose, an action describing the transition, and an `image_size` both poses have to match: 16 to 128 per side, half the reach of the animation routes. It carries no frame count and returns four to eight frames of its own choosing. Each pose is a `KeyframeImage` — `{image, size}` — rather than the bare `Base64Image` every other slot takes.
