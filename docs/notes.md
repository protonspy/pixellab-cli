...
- n-0061 2026-09-20 #gotcha @src/pixellab_cli/subject.py — a state can be made from another state, so the subject record walks the source chain to the root character; a pose filed under nothing is a guardrail that stops seeing it
- n-0062 2026-09-20 #ceiling @src/pixellab_cli/fal.py — fal's balance comes from the undocumented rest.alpha.fal.ai/billing/user_balance because the documented v1/account/billing needs an admin-scope key; a failure prints fal: not reported
